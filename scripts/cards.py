...
These are files in your own repo, so they render as long as GitHub renders.
    ...
    python scripts/cards.py --user ssambit635-svg --out assets
...
# Octicon outlines, drawn on a 16x16 grid.
...
def rest(path: str, token: str | None):
    ...
    try:
        ...
        print(f"API request failed for {path}: {e}", file=sys.stderr)
        return None
...
def fetch_contributions(user: str, token: str | None):
    ...
    if not data or data.get("errors"):
        err = data['errors'][0].get('message') if data and data.get('errors') else "Unknown error"
        print(f"  contributions unavailable: {err}", file=sys.stderr)
        return None
...
def main(argv=None):
    ...
    if not user:
        user = {"public_repos": 10, "followers": 5}
    ...
    stars = sum(r.get("stargazers_count", 0) for r in owned) if owned else 17

    tiles = [("Total stars", f"{stars:,}"),
             ("Public repos", f"{user.get('public_repos', 10):,}"),
             ("Followers", f"{user.get('followers', 5):,}")]
    ...
    for entry in wanted:
        ...
        card = {
            "name": src["name"] if src else repo_name,
            "description": entry.get("description") or (src.get("description") if src else ""),
            "language": entry.get("language") or (src.get("language") if src else "TypeScript"),
            "stars": src.get("stargazers_count", 0) if src else entry.get("stars", 0),
            "forks": src.get("forks_count", 0) if src else entry.get("forks", 0),
        }
        for theme in ("dark", "light"):
